...
namespace monitor
{
    void CpuStatMonitor::UpdataOnce(monitor::proto::MonitorInfo *monitor_info)
    {
        ReadFile cpt_stat_file(std::string("/proc/stat"));
        std::vector<std::string> cpu_stat_list;
        while (cpt_stat_file.ReadLine(&cpu_stat_list))
        {
            if (cpu_stat_list[0].find("cpu") != std::string::npos)
            {
                ...
                auto it = _cpu_stat.find(cpu_stat.cpu_name);
                if (it != _cpu_stat.end())
                {
                    struct CpuStat old = it->second;
                    monitor::proto::CpuStat *cpu_stat_msg = monitor_info->add_cpu_stat();
                    float new_cpu_total_time = cpu_stat.user + cpu_stat.nice +
                                               cpu_stat.system + cpu_stat.idle +
                                               cpu_stat.io_wait + cpu_stat.irq +
                                               cpu_stat.soft_irq + cpu_stat.steal;
                    float old_cpu_total_time = old.user + old.nice +
                                               old.system + old.idle +
                                               old.io_wait + old.irq +
                                               old.soft_irq + old.steal;
                    float new_cpu_busy_time = cpu_stat.user + cpu_stat.nice +
                                              cpu_stat.system + cpu_stat.irq +
                                              cpu_stat.soft_irq + cpu_stat.steal;
                    float old_cpu_busy_time = old.user + old.nice +
                                              old.system + old.irq +
                                              old.soft_irq + old.steal;
                    ...
                    cpu_stat_msg->set_cpu_precent(cpu_percent);
                    cpu_stat_msg->set_user_precent(cpu_user_percent);
                    cpu_stat_msg->set_idle_precent(cpu_idle_percent);
                    cpu_stat_msg->set_system_precent(cpu_system_percent);
                    cpu_stat_msg->set_iowait_precent(cpu_io_wait_percent);
                    cpu_stat_msg->set_irq_precent(cpu_irq_percent);
                    cpu_stat_msg->set_softirp_precent(cpu_soft_irq_percent);
                }
