...
namespace monitor
{
    ...
    void MemMonitor::UpdataOnce(monitor::proto::MonitorInfo *monitor_info)
    {
        ReadFile mem_file("/proc/meminfo");
        struct MenInfo mem_info;
        std::vector<std::string> mem_data;
        while (mem_file.ReadLine(&mem_data))
        {
            if (mem_data[0] == "MemTotal:")
            {
                mem_info.total = std::stoll(mem_data[1]);
            }
            else if (mem_data[0] == "MemFree:")
            {
                mem_info.free = std::stoll(mem_data[1]);
            }
            else if (mem_data[0] == "MemAvailable:")
            {
                mem_info.avail = std::stoll(mem_data[1]);
            }
            else if (mem_data[0] == "Buffers:")
            {
                mem_info.buffers = std::stoll(mem_data[1]);
            }
            else if (mem_data[0] == "Cached::")
            {
                mem_info.cached = std::stoll(mem_data[1]);
            }
            else if (mem_data[0] == "SwapCached:")
            {
                mem_info.swap_cached = std::stoll(mem_data[1]);
            }
            else if (mem_data[0] == "Active:")
            {
                mem_info.active = std::stoll(mem_data[1]);
            }
            else if (mem_data[0] == "Inactive:")
            {
                mem_info.in_active = std::stoll(mem_data[1]);
            }
            else if (mem_data[0] == "Active(anon):")
            {
                mem_info.active_anon = std::stoll(mem_data[1]);
            }
            else if (mem_data[0] == "Inactive(anon):")
            {
                mem_info.inactive_anon = std::stoll(mem_data[1]);
            }
            else if (mem_data[0] == "Active(file):")
            {
                mem_info.active_file = std::stoll(mem_data[1]);
            }
            else if (mem_data[0] == "Inactive(file):")
            {
                mem_info.inactive_file = std::stoll(mem_data[1]);
            }
            else if (mem_data[0] == "Dirty:")
            {
                mem_info.dirty = std::stoll(mem_data[1]);
            }
            else if (mem_data[0] == "Writeback:")
            {
                mem_info.writeback = std::stoll(mem_data[1]);
            }
            else if (mem_data[0] == "AnonPages:")
            {
                mem_info.anon_pages = std::stoll(mem_data[1]);
            }
            else if (mem_data[0] == "Mapped:")
            {
                mem_info.mapped = std::stoll(mem_data[1]);
            }
            else if (mem_data[0] == "KReclaimable:")
            {
                mem_info.kReclaimable = std::stoll(mem_data[1]);
            }
            else if (mem_data[0] == "SReclaimable:")
            {
                mem_info.sReclaimable = std::stoll(mem_data[1]);
            }
            else if (mem_data[0] == "SUnreclaim:")
            {
                mem_info.sUnreclaim = std::stoll(mem_data[1]);
            }
            mem_data.clear();
        }
        ...
        mem_detail->set_buffer(mem_info.buffers / KBToGB);
